...
unsigned short GetCrc(unsigned char *Buf, unsigned short lastcrc, unsigned long count)
{
	...
	unsigned long i;
      ...
      for( i = 0; i < count ; i++)
      {
        crc = (((crc) >> 8) ^ CrcTable[((crc) ^ (Buf[i])) & 0x00ff]);
      }						
      return(crc);
}
...
{
   unsigned short crc = (((lastcrc) >> 8) ^ CrcTable[((lastcrc) ^ (ch)) & 0x00ff]);
   return(crc);
}

void AppendCrc(unsigned char *crcData, unsigned short lastcrc, unsigned long length)
{
 unsigned int crc =  GetCrc(crcData, lastcrc, length);
 crcData[length+1] = (crc &0xff00)>>8;
 crcData[length] = (crc &0xff);
}

bool CheckCrc(unsigned char *crcData, unsigned short lastcrc, unsigned long length)
{
  return(GetCrc(crcData, lastcrc, length) == (unsigned short)0);
}
